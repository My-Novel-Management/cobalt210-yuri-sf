...
def fishes(w: World):
    ...
    return w.scene("水槽の魚たち",
            ...
            luna.do("声を聞かなくても、端末を向けて認識しなくても、それが委員長だとすぐに分かる",
                ...
                "その立ち振舞いと$Mを見つけたときの安堵とどうしようもなさを織り交ぜて僅かに両肩を落とす仕草は、紛れもなく彼女だけのものだ"),
            luna.talk("$ln_yokoさん",
                "放っておいてくれればいいのに",
                "別にあなたに迷惑を掛けていないわ"),
...
def out_suites(w: World):
    ...
    return w.scene("スーツを脱いで",
            ...
            luna.do("覚悟を決めた$yokoに黙って頷くと、$Mは背を向けた",
                    ...
                    "それが当たり前になっていた",
                    "けれどそんな当たり前を当たり前にしてしまうことは不幸なことなんだって誰かが気づかなきゃいけない",
                    "少なくとも$Mはそれを疑う自分でいたい"),
            luna.talk("もういい？"),
            luna.do("返事はなかった",
                    "一瞬本当に脱いでくれているのだろうかと不安になったが「うん」という小さな声と共に背中に彼女の手が当てられ、$Mはゆっくりと振り返る"),
            luna.talk("$yoko……"),
            luna.do("そこには自分の左腕を抱える下着姿の彼女がいた"),
            luna.talk("ごめん"),
            luna.do("腰までのブラトップに下は本当に小さなショーツ一枚だ",
                    "$Mは慌てて自分の制服を脱ぐと、それを彼女に着るように言った"),
            yoko.talk("え、けど"),
            luna.talk("いいの", "本当は新しいのを$yokoに着てもらいたかったんだけど、今はこれしかないから",
                    "嫌だったら、その"),
            yoko.talk("嫌じゃない"),
            luna.do("彼女の手が制服の上着を脱ごうとしていた$Mの腕を掴む",
                    "その二重の瞳は$Mを真っ直ぐに捉えて離さなくて、その勢いに気圧されるままに脱いだ制服を$yokoに渡した"),
            yoko.talk("うん"),
            luna.do("三分ほどして聞こえたのは「いいよ」ではなくただの頷きだった"),
            luna.talk("あ……"),
            luna.do("部屋の明かりが戻る",
                    "停電から復旧したのだ",
                    "$Mの眼の前には夢にまで見たセーラー服姿の$yokoが立っていた",
                    "少し目線を右にして俯き、ぎゅっと握った手を所在なくスカートの上に置いている",
                    "その仕草がまたとても似合っていて$Mは言葉を失ったまま彼女に抱きついた"),
            yoko.talk("$ln_lunaさん？"),
            luna.talk("なんか、想像以上で"),
            yoko.talk("え？"),
            luna.talk("$meがずっと見たかったものが今目の前にあって、たぶん、人生で一番嬉しい"),
            luna.do("頬を赤らめた$yokoはそのまま一枚の絵のようで、$Mは水槽の中のメダカが一匹お腹を向けて浮いているのも全然目に入らず、ただただ彼女だけを見つめていた"),
            )
...
def alone(w: World):
    ...
    return w.scene('空虚な水槽',
            ...
            luna.do("$Mはその日も授業をサボり、水質管理室にいた",
                "けれど目の前の水槽にはメダカどころか生き物は何もなくて、ただ水が存在しているだけだった"),
            luna.do("小さく張り紙がされ『使用不可』とだけ乱雑な字で書かれたそれは、$Mに何も教えてくれない"),
            luna.do("$Mはヘルメットを取り、防護スーツを脱ぐ",
                "握った右手をゆっくり開くと、そこには今まで見たことのない黒いシミが二つ、大きな黒子のようにして付いていた"),
            )
